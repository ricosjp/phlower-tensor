...
def phlower_tensor_collection(
    values: dict[str, torch.Tensor] | dict[str, PhlowerTensor],
) -> IPhlowerTensorCollections:
...
class PhlowerDictTensors(IPhlowerTensorCollections):
    def __init__(
        self, value: dict[str, torch.Tensor] | dict[str, PhlowerTensor]
    ):
    ...
    def __lt__(
        ...
        if isinstance(__value, IPhlowerTensorCollections):
    ...
    def slice(
        ...
        tmp = {k: v[slice_range] for k, v in self._data.items()}
    ...
    def sum(self, weights: dict[str, float] | None = None) -> PhlowerTensor:
        return cast(PhlowerTensor, torch.sum(_all_stack(self, weights=weights)))  # type: ignore[call-overload]

    def mean(self, weights: dict[str, float] | None = None) -> PhlowerTensor:
        if weights is None:
            return cast(
                PhlowerTensor,
                torch.mean(_all_stack(self)),  # type: ignore[call-overload]
            )

        total_weight = sum(weights.values())
        return cast(
            PhlowerTensor,
            torch.sum(_all_stack(self, weights)) / total_weight,  # type: ignore[call-overload]
        )
...
def reduce_stack(
    ...
    result = {
        k: phlower_tensor(
            torch.stack(v),  # type: ignore[arg-type]
