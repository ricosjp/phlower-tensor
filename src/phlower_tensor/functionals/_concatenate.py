...
def concatenate(
    tensors: Sequence[PhlowerTensor], dense_dim: int = 0
) -> PhlowerTensor:
...
def _dense_concatenate(
    tensors: Sequence[PhlowerTensor], dim: int = 0
) -> PhlowerTensor:
    return torch.concatenate(tensors, dim=dim)  # type: ignore[call-overload]


def _sparse_concatenate(
    tensors: Sequence[PhlowerTensor], dimension: PhlowerDimensionTensor | None
) -> PhlowerTensor:
