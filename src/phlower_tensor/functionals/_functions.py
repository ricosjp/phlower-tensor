...
class BNFRestoreInfo(NamedTuple):
    ...
    bnf_pattern: str
    restore_axes_length: dict[str, int]
...
def from_batch_node_feature(
    ...
    Returns:
        resultant_tensor: PhlowerTensor
            Resultant tensor.
    """
    ...
    *args: PhlowerTensor,
    ...
    try:
        ...
        raise PhlowerIncompatibleTensorError(
            f"{e}\n{equation}, {[a.shape for a in args]}"
        ) from e
...
def contraction(
    x: PhlowerTensor, y: PhlowerTensor | None = None
) -> PhlowerTensor:
    """
...
def tensor_product(x: PhlowerTensor, y: PhlowerTensor) -> PhlowerTensor:
    """
...
def tensor_times_scalar(
    tensor: PhlowerTensor, scalar: int | float | PhlowerTensor
) -> PhlowerTensor:
    """
    Compute multiplication between tensor and scalar (field).

    Args:
        tensor: PhlowerTensor
            Input tensor.
        scalar: int | float | PhlowerTensor
...
def apply_orthogonal_group(
    orthogonal_matrix: PhlowerTensor, tensor: PhlowerTensor
) -> PhlowerTensor:
    """
    Apply orthogonal group action to the input tensor.

    Args:
        orthogonal_matrix: PhlowerTensor
...
def spatial_sum(tensor: PhlowerTensor) -> PhlowerTensor:
    ...
    for _ in range(space_width):
        squeezed = cast(PhlowerTensor, torch.unsqueeze(squeezed, start_space))  # type: ignore[arg-type]

    return squeezed.as_pattern(f"{time}{space}...")


def spatial_mean(
    tensor: PhlowerTensor, weight: PhlowerTensor | None = None
) -> PhlowerTensor:
...
def squeeze(input: PhlowerTensor, dim: int | None = None) -> PhlowerTensor:
    """
