...
from phlower_tensor.utils import get_logger
...
def phlower_tensor(
    ...
    pattern: PhlowerShapePattern | str | None = None,
...
def _resolve_dimension_arg(
    ...
    raise NotImplementedError(
        f"{type(inputs)} cannot be converted to PhlowerDimensionTensor"
    )
...
class PhlowerTensor:
    ...
    def __eq__(self, other: PhlowerTensor | torch.Tensor | float | int):
        return torch.eq(self, other)

    def __lt__(self, other: PhlowerTensor | torch.Tensor | float | int):
        return torch.lt(self, other)

    def __le__(self, other: PhlowerTensor | torch.Tensor | float | int):
        return torch.le(self, other)

    def __gt__(self, other: PhlowerTensor | torch.Tensor | float | int):
        return torch.gt(self, other)

    def __ge__(self, other: PhlowerTensor | torch.Tensor | float | int):
        return torch.ge(self, other)

    def __abs__(self) -> PhlowerTensor | torch.Tensor | float | int:
        return torch.abs(self)

    def __sub__(self, other: PhlowerTensor | torch.Tensor | float | int):
        return torch.sub(self, other)

    def __rsub__(self, other: PhlowerTensor | torch.Tensor | float | int):
