...
class PhlowerShapePattern:
    ...
    def from_pattern(
        ...
        if not _check_shape_and_pattern(shape, _splited):
            raise PhlowerIncompatibleTensorError(
                f"Invalid tensor shape and pattern. shape: {shape}, "
                f"pattern: {pattern}"
            )
    ...
    def get_n_vertices(self) -> int:
        ...
        if self._is_voxel:
            return np.prod(self._shape[start : start + 3]).item()
...
class _IndexKeyWrapper:
    def __init__(self, keys: _ACCESS_ITEM, shapes: torch.Size):
        _keys = keys if isinstance(keys, list | tuple) else [keys]
        self._key = self._resolve_ellipse(_keys, shapes)
        self._shapes = shapes

        self._first_none_index = self._find_first_none_index()

    def _find_first_none_index(self) -> float:
    ...
    def __getitem__(self, index: int) -> _ACCESS_ITEM:
