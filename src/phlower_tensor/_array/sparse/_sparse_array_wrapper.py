...
from phlower_tensor.utils.typing import SparseArrayType

logger = get_logger(__name__)


class SparseArrayWrapper(IPhlowerArray[SparseArrayType]):
    def __init__(
    ...
    def apply(
        self,
        function: Callable[[SparseArrayType], SparseArrayType],
        componentwise: bool,
        *,
        skip_nan: bool = False,
        ...
        **kwards,
        ...
        result = function(reshaped_array).reshape(*self.shape).tocoo()
    ...
    def _reshape(
        ...
        if use_diagonal:
            # NOTE: scipy-stubs' problem ?
            return self._sparse_data.diagonal()  # type: ignore[return-value]
